...
chi = np.r_[1., 0., 1.]              # susceptibility

mu0 = 4*np.pi*1e-7
min_skin_depth = (np.pi * np.max(mu0 * (1+chi) * sigma) * frequencies.min())**-0.5
max_skin_depth = (np.pi * np.min(mu0 * (1+chi) * sigma) * frequencies.max())**-0.5

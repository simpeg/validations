...
rootdir = './../../../assets/tdem/layered_earth_susceptible_fwd'
mesh_simpeg = TreeMesh.readUBC(rootdir+'_simpeg/octree_mesh.txt')
con_model_simpeg = TreeMesh.readModelUBC(mesh_simpeg, rootdir+'_simpeg/model.con')
sus_model_simpeg = TreeMesh.readModelUBC(mesh_simpeg, rootdir+'_simpeg/model.sus')
...
mesh_simpeg.plot_slice(
    plotting_map * log_model, normal="Y", ax=ax1, ind=int(mesh_simpeg.hy.size / 2),
    clim=(np.min(log_model)-1, np.max(log_model)+1), grid=True
)
...
norm = mpl.colors.Normalize(
    vmin=np.min(log_model)-1, vmax=np.max(log_model)+1
)
...
ax1 = fig.add_axes([0.14, 0.1, 0.6, 0.85])
mesh_simpeg.plot_slice(
    plotting_map * sus_model_simpeg[ind_active], normal="Y", ax=ax1, ind=int(mesh_simpeg.hy.size / 2),
    clim=(np.min(sus_model_simpeg), np.max(sus_model_simpeg)), grid=True, pcolorOpts={'cmap':'plasma'}
)

...
# 
# For this code comparison, we inverted total magnetic intensity (TMI) data collected over a block within a homogeneous halfspace. For each coding package, we invert for a compact and blocky model using an iteratively re-weighted least-squares inversion approach. The problem is bounded to enforce positivity in the recovered susceptibility model.
# 
...
rootdir = './../../../assets/magnetics/block_halfspace_tmi_inv_sparse_mag3d/'
mesh_ubc = TensorMesh.read_UBC(rootdir+'mesh.txt')
true_model_ubc = TensorMesh.read_model_UBC(mesh_simpeg, rootdir+'true_model.sus')
recovered_model_ubc = TensorMesh.read_model_UBC(mesh_simpeg, rootdir+'maginv3d_014.sus')
dobs_ubc = read_mag3d_ubc(rootdir+'dobs.mag')
dpred_ubc = read_mag3d_ubc(rootdir+'maginv3d_014.pre')

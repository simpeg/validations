...
# 
# For this code comparison, we inverted gravity anomaly data collected over a block within a homogeneous halfspace. For each coding package, we recovered the smoothest model using an unconstrained least-squares inversion approach.
# 
...
rootdir = './../../../assets/gravity/block_halfspace_gravity_inv_smooth_grav3d/'
mesh_ubc = TensorMesh.read_UBC(rootdir+'mesh.txt')
true_model_ubc = TensorMesh.read_model_UBC(mesh_simpeg, rootdir+'true_model.den')
recovered_model_ubc = TensorMesh.read_model_UBC(mesh_simpeg, rootdir+'gzinv3d_002.den')
dobs_ubc = read_grav3d_ubc(rootdir+'dobs.grv')
dpred_ubc = read_grav3d_ubc(rootdir+'gzinv3d_002.pre')

...
# 
# **Link to the docstrings for the simulation.** The docstrings will have a citation and show the formulation.
...
xyz_tx = np.c_[0., 0., 5.]           # Dipole source location
...
# A compressed folder containing additional assets required to run the notebook is then downloaded. This includes mesh and model files for the forward simulation.
...
mesh = TreeMesh.read_UBC(meshfile)
sigma_model = TreeMesh.read_model_UBC(mesh, modelfile)
...
# Here we define the survey object used in the forward simulation.
...
# If desired, we can export the data to xyz format
